"""
Dream Cricket 25 - ULTRA Farmer v6.1 🌌 GALAXY EDITION
Theme: Purple Cosmic Galaxy
- Pura Galaxy theme — intro + login + main page
- Dual Job System (A + B simultaneously)
- Real-time counters fixed
- Auto-chain Legendary + Champion
- Token Vault, Auto-ping, BG Jobs
"""
...
def run_phase(slot,hdr,total,workers,fn,phase_key):
    lk=slots[slot]["lock"]
    batches=math.ceil(total/workers); bt=[]; phase_done=0
...
def run_job(slot):
    ...
    with lk:
        j=slots[slot]["job"]
        if not j: return
        token=j["token"]; total=j["total"]; workers=j["workers"]; mode_key=j["mode_key"]
    hdr=make_headers(token); m=MODES[mode_key]
    if m["type"]=="chain":
        elite_needed=total*m["elite_per_card"]
        with lk:
            j=slots[slot]["job"]
            if j: j["phase"]=1; j["phase1_total"]=elite_needed; j["phase2_total"]=total; j["phase_done"]=0
        p1ok=run_phase(slot,hdr,elite_needed,workers,lambda:do_elite(hdr),"phase1_success")
        with lk:
            j=slots[slot]["job"]
            if not j or not j["running"]: _finish(slot,mode_key,total,workers); return
            j["phase"]=2; j["phase_done"]=0
        cards=p1ok//m["elite_per_card"]
        run_phase(slot,hdr,cards,max(1,workers//5),lambda:do_exchange(hdr,mode_key),"phase2_success")
    else:
        with lk:
            j=slots[slot]["job"]
            if j: j["phase_done"]=0
        run_phase(slot,hdr,total,workers,lambda:do_reward(hdr,mode_key),"success")
...
def _finish(slot,mode_key,total,workers):
    ...
    with lk:
        ...
        elapsed=j["end_time"]-j["start_time"]; m=MODES[mode_key]
        success=j.get("phase2_success",0) if m["type"]=="chain" else j.get("success",0)
        entry={"reward":success*m["amount"],"unit":m["unit"],"label":m["label"],"mode_key":mode_key,
            "success":success,"total":total,"workers":workers,"elapsed":round(elapsed,1),
            "time":datetime.now().strftime("%H:%M:%S"),"date":datetime.now().strftime("%d %b")}
        slots[slot]["history"].insert(0,entry)
        if len(slots[slot]["history"])>5: slots[slot]["history"]=slots[slot]["history"][:5]
...
def get_slot_status(slot):
    ...
    if not j:
        return {"running":False,"done":False,"slot":slot,"pct":0,"reward":0,"unit":"",
            "success":0,"fail":0,"eta":0,"speed":0,"elapsed":0,"speed_history":[],
            "history":hist,"has_active":False,"phase":1,"phase1_total":0,"phase2_total":0,
            "phase_done":0,"phase1_success":0,"phase2_success":0,"is_chain":False,"mode_key":"gems","total":0}
    ...
    return {"running":j["running"],"done":j["done"],"slot":slot,"pct":pct,"reward":reward,
        "unit":m["unit"],"label":m["label"],"success":success,"fail":j.get("fail",0),
        "eta":round(j.get("eta",0)),"speed":j.get("speed",0),"elapsed":round(elapsed),
        "speed_history":j.get("speed_history",[]),"history":hist,"has_active":j["running"],
        "mode_key":mk,"is_chain":is_chain,"total":j["total"],
...
def start():
    ...
    m=MODES[mode_key]; total=desired if m["type"]=="chain" else math.ceil(desired/m["amount"])
    with lk:
        slots[slot]["job"]={"running":True,"done":False,"success":0,"fail":0,"total":total,
...
.gring:nth-child(1){width:160px;height:160px;animation-delay:0s;}
...
.galaxy-orb{width:90px;height:90px;border-radius:50%;background:radial-gradient(circle at 35% 35%,#9933ff,#4400aa,#220055);box-shadow:0 0 50px rgba(194,100,254,.7),inset 0 0 25px rgba(255,255,255,.1);animation:orbPulse 3s ease-in-out infinite;}
...
.mt{font-family:'Orbitron',monospace;font-weight:900;font-size:clamp(20px,6vw,44px);letter-spacing:8px;color:#c264fe;text-shadow:0 0 30px rgba(194,100,254,.8),0 0 60px rgba(194,100,254,.3);position:relative;}
...
.pf{height:100%;width:0%;background:linear-gradient(90deg,#220055,#9933ff,#c264fe);box-shadow:0 0 10px rgba(194,100,254,.4);}
...
.agt{font-family:'Orbitron',monospace;font-weight:900;font-size:clamp(14px,4vw,28px);letter-spacing:8px;color:#c264fe;text-shadow:0 0 30px rgba(194,100,254,1);}
...
.lc{background:#0d0018;border:1px solid rgba(194,100,254,.2);border-radius:8px;padding:32px 26px 28px;width:100%;max-width:360px;position:relative;overflow:hidden;margin:18px;}
.lc::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#c264fe,#9933ff,#c264fe,transparent);}
.lc::after{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%,rgba(194,100,254,.08) 0%,transparent 60%);pointer-events:none;}
.ll{text-align:center;margin-bottom:22px;}
.li{font-size:38px;margin-bottom:10px;filter:drop-shadow(0 0 15px rgba(194,100,254,.8));}
.lh{font-family:'Orbitron',monospace;font-weight:900;font-size:17px;letter-spacing:6px;background:linear-gradient(135deg,#c264fe,#ff44ff,#9933ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.lp{font-size:9px;color:#4a2a6a;letter-spacing:4px;margin-top:4px;}
.lf{margin-bottom:13px;}.lf label{display:block;font-size:9px;color:#4a2a6a;letter-spacing:3px;margin-bottom:5px;}
.lf input{width:100%;background:rgba(194,100,254,.03);border:1px solid rgba(194,100,254,.15);border-radius:4px;color:#c264fe;font-family:'Share Tech Mono',monospace;font-size:14px;padding:11px 13px;outline:none;transition:border-color .2s;}
.lf input:focus{border-color:#c264fe;box-shadow:0 0 12px rgba(194,100,254,.15);}
.lb{width:100%;padding:14px;margin-top:5px;background:linear-gradient(135deg,#1a0028,#350050,#28003a);border:1px solid #9933ff;border-radius:6px;color:#c264fe;font-family:'Orbitron',monospace;font-weight:900;font-size:12px;letter-spacing:5px;cursor:pointer;position:relative;overflow:hidden;text-shadow:0 0 15px rgba(194,100,254,.6);}
.lb::before{content:'';position:absolute;top:-50%;left:-60%;width:30%;height:200%;background:linear-gradient(90deg,transparent,rgba(194,100,254,.15),transparent);transform:skewX(-20deg);animation:sh 3s infinite;}
@keyframes sh{0%{left:-60%}100%{left:160%}}
.lb:disabled{opacity:.4;cursor:not-allowed;}
.le{display:none;background:rgba(255,34,68,.06);border:1px solid rgba(255,34,68,.2);border-radius:4px;color:#ff4466;font-size:11px;padding:9px;margin-top:9px;text-align:center;}
...
<canvas id="cv"></canvas>
<!-- Nebula -->
<div class="nebula" style="width:400px;height:400px;background:#6600aa;top:10%;left:20%;animation-delay:0s;"></div>
<div class="nebula" style="width:300px;height:300px;background:#3300ff;bottom:15%;right:20%;animation-delay:4s;"></div>
<div id="intro">
  ...
  <div id="ctr">
    <div class="orb-wrap">
      ...
      <div class="orb-gem">💎</div>
    </div>
    <div class="mt">GALAXY FARM</div>
    <div class="vt">COSMIC MODE · DC25 · v6.1</div>
    <div id="pw">
      <div class="ph"><span id="pl">INITIALIZING...</span><span id="pn">0%</span></div>
      <div class="pt2"><div class="pf" id="pfl"></div></div>
      <div class="sps">
        <div class="sr"><span class="sl2">DUAL JOBS</span><div class="sts"><div class="sf" id="s1"></div></div><span class="sp" id="s1p">0%</span></div>
        <div class="sr"><span class="sl2">GALAXY ENGINE</span><div class="sts"><div class="sf" id="s2" style="background:#7722cc"></div></div><span class="sp" id="s2p">0%</span></div>
        <div class="sr"><span class="sl2">AUTO-CHAIN</span><div class="sts"><div class="sf" id="s3" style="background:#ff44ff"></div></div><span class="sp" id="s3p">0%</span></div>
...
for(let i=0;i<150;i++){
  ...
  s.style.cssText=`width:${sz}px;height:${sz}px;top:${Math.random()*100}%;left:${Math.random()*100}%;animation-delay:${Math.random()*3}s;animation-duration:${2+Math.random()*3}s;opacity:${Math.random()*.6+.1}`;
...
function drawM(){
  cx.fillStyle='rgba(5,0,10,0.06)';cx.fillRect(0,0,cv.width,cv.height);
  drops.forEach((y,i)=>{
    const c=CH[Math.floor(Math.random()*CH.length)];
    const r=Math.random();
    cx.fillStyle=r>.95?'#ff88ff':r>.8?'#c264fe':r>.5?'#6600aa44':'#33006622';
    cx.font='13px Share Tech Mono';cx.fillText(c,i*16,y*16);
    if(y*16>cv.height&&Math.random()>.975)drops[i]=0;drops[i]++;
  });
}
const mI=setInterval(drawM,50);
let pct=0,li=0;
const lbls=['WARPING...','DUAL JOBS...','GALAXY ENGINE...','AUTO-CHAIN...','COSMIC READY!'];
...
function showAG(){
  const ag=document.getElementById('ag');ag.style.opacity='1';ag.style.pointerEvents='auto';
  let f=0;const iv=setInterval(()=>{f++;ag.style.background=f%2===0?'#05000a':'rgba(194,100,254,.06)';if(f>=6){clearInterval(iv);setTimeout(showLogin,500);}},130);
}
function showLogin(){
  [document.getElementById('intro'),document.getElementById('ag')].forEach(el=>{el.style.transition='opacity 0.7s';el.style.opacity='0';});
  document.getElementById('sk').style.display='none';clearInterval(mI);
  setTimeout(()=>{document.getElementById('intro').style.display='none';document.getElementById('ag').style.display='none';document.getElementById('lw').classList.add('show');document.getElementById('em').focus();},750);
}
...
<style>
:root{
  --bg:#05000a;--panel:#0d0018;--border:rgba(194,100,254,.15);
  --p1:#c264fe;--p2:#9933ff;--p3:#6600aa;--p4:#330055;
  --p1a:rgba(194,100,254,.08);
  --pink:#ff44ff;--blue:#4400ff;--cyan:#aa44ff;
  --red:#ff2244;--yellow:#ffd600;--gold:#ffd700;--orange:#ff8c00;
  --text:#d4a8ff;--dim:#4a2a6a;
  /* mode colors */
  --cgems:#c264fe;--ctickets:#ff8c00;--celite:#aa88ff;
  --clegend:#ff44ff;--cchamp:#ffd700;
}
*{box-sizing:border-box;margin:0;padding:0;}
body{background:var(--bg);color:var(--text);font-family:'Rajdhani',sans-serif;min-height:100vh;overflow-x:hidden;}
/* Star field */
body::before{content:'';position:fixed;inset:0;background:
  radial-gradient(ellipse 90% 60% at 20% 10%,rgba(102,0,170,.12) 0%,transparent 60%),
  radial-gradient(ellipse 70% 50% at 80% 90%,rgba(68,0,255,.08) 0%,transparent 60%),
  radial-gradient(ellipse 50% 40% at 50% 50%,rgba(194,100,254,.04) 0%,transparent 70%);
...
.ds-item.sa{border-left:2px solid var(--p1);}
.ds-item.sb{border-left:2px solid var(--pink);}
...
.cg.p{background:linear-gradient(90deg,transparent,var(--p1),transparent);}
.cg.pk{background:linear-gradient(90deg,transparent,var(--pink),transparent);}
.cg.cy{background:linear-gradient(90deg,transparent,var(--cyan),transparent);}
.cg.go{background:linear-gradient(90deg,transparent,var(--gold),transparent);}
.cg.or{background:linear-gradient(90deg,transparent,var(--orange),transparent);}
...
.tok-area:focus{border-color:var(--p1);box-shadow:0 0 10px rgba(194,100,254,.1);}
...
input[type=number]{width:100%;background:rgba(194,100,254,.03);border:1px solid var(--border);border-radius:4px;color:var(--p1);font-family:'Share Tech Mono',monospace;font-size:14px;padding:8px 9px;outline:none;transition:border-color .2s;}
...
/* Buttons */
.btn-start{width:100%;padding:13px;background:linear-gradient(135deg,#1a0028,#350050,#28003a);border:1px solid var(--p2);border-radius:6px;color:var(--p1);font-family:'Orbitron',monospace;font-weight:900;font-size:11px;letter-spacing:4px;cursor:pointer;transition:all .2s;position:relative;overflow:hidden;text-shadow:0 0 15px rgba(194,100,254,.6);}
...
.btn-start:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 4px 20px rgba(194,100,254,.2);}
...
.bar-fill{height:100%;background:linear-gradient(90deg,var(--p4),var(--p2),var(--p1));width:0%;transition:width .4s;box-shadow:0 0 8px rgba(194,100,254,.3);}
...
.done-card::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%,rgba(194,100,254,.1) 0%,transparent 60%);pointer-events:none;}
...
<div class="hdr">
  ...
  <p>DUAL JOBS · AUTO-CHAIN · TOKEN VAULT · v6.1</p>
  <button class="logout-btn" onclick="doLogout()">⏻ EXIT</button>
</div>

<!-- Dual Status -->
<div class="dual-status">
  <div class="ds-item sa">
    <div class="ds-slot">⚡ JOB A</div>
    <div class="ds-reward" id="dsAR">—</div>
    <div class="ds-info" id="dsAI">Idle</div>
    <div class="ds-bar"><div class="dfa" id="dsAF" style="width:0%"></div></div>
    <button class="ds-stop" id="dsAS" onclick="stopSlot('A')" style="display:none">⛔</button>
  </div>
  <div class="ds-item sb">
    <div class="ds-slot">⚡ JOB B</div>
    <div class="ds-reward" id="dsBR">—</div>
    <div class="ds-info" id="dsBI">Idle</div>
    <div class="ds-bar"><div class="dfb" id="dsBF" style="width:0%"></div></div>
    <button class="ds-stop" id="dsBS" onclick="stopSlot('B')" style="display:none">⛔</button>
  </div>
</div>
...
<div class="job-tabs">
  <div class="job-tab aa" id="tabA" onclick="switchTab('A')">
    <div class="jtl"><span class="jbadge idle" id="badgeA"></span>JOB A</div>
    <div class="jts" id="subA">Ready</div>
  </div>
  <div class="job-tab" id="tabB" onclick="switchTab('B')">
    <div class="jtl"><span class="jbadge idle" id="badgeB"></span>JOB B</div>
    <div class="jts" id="subB">Ready</div>
  </div>
</div>
...
<div class="job-panel show" id="panelA">
  <div class="card"><div class="cg p"></div><div class="slot-badge a">🌌 JOB SLOT A</div><div class="sec">// TOKEN A</div>
  ...
  <div class="card"><div class="cg go"></div><div class="sec">// MODE A</div><div class="mode-grid" id="mgA"></div><div class="chainBox" id="cbA"><div class="chain-title" id="ctA">⚡ AUTO-CHAIN</div><div class="chain-steps"><div class="cs"><div class="cs-icon">🃏</div><div class="cs-lbl">ELITE</div></div><div class="cs-arr">→</div><div class="cs"><div class="cs-icon" id="ciA">⭐</div><div class="cs-lbl">EXCHANGE</div></div><div class="cs-arr">→</div><div class="cs"><div class="cs-icon" id="ci2A">⭐</div><div class="cs-lbl">DONE!</div></div></div><div class="chain-calc" id="ccA">Enter amount</div></div></div>
  <div class="card"><div class="cg cy"></div><div class="sec">// CONFIG A</div><div class="cfg-row"><div class="cfg-item"><label id="dlA">💎 DESIRED</label><input type="number" id="desA" value="5000" min="1" step="2" oninput="upCalc('A')"/><div class="hint" id="htA">1 click = 2 gems</div></div><div class="cfg-item"><label>⚡ WORKERS</label><input type="number" id="wrkA" value="20" min="1" max="200"/><div class="presets" id="preA"></div></div></div><div class="bg-notice">🌌 Background mein chalta hai — phone band karo!</div></div>
  ...
  <div class="card prog-card" id="progA"><div class="cg p"></div><div class="sec">// JOB A PROGRESS</div>
    <div class="prog-top">
      <div class="ring-wrap">
  ...
  <div class="card"><div class="cg pk"></div><div class="sec">// JOB A HISTORY</div><div class="sess-list" id="histA"><div class="empty">NO JOBS YET <span class="blink">_</span></div></div></div>
</div>

<!-- ═══ JOB B ═══ -->
<div class="job-panel" id="panelB">
  <div class="card"><div class="cg pk"></div><div class="slot-badge b">🌌 JOB SLOT B</div><div class="sec">// TOKEN B</div>
  ...
  <div class="card"><div class="cg go"></div><div class="sec">// MODE B</div><div class="mode-grid" id="mgB"></div><div class="chainBox" id="cbB"><div class="chain-title" id="ctB">⚡ AUTO-CHAIN</div><div class="chain-steps"><div class="cs"><div class="cs-icon">🃏</div><div class="cs-lbl">ELITE</div></div><div class="cs-arr">→</div><div class="cs"><div class="cs-icon" id="ciB">⭐</div><div class="cs-lbl">EXCHANGE</div></div><div class="cs-arr">→</div><div class="cs"><div class="cs-icon" id="ci2B">⭐</div><div class="cs-lbl">DONE!</div></div></div><div class="chain-calc" id="ccB">Enter amount</div></div></div>
  <div class="card"><div class="cg cy"></div><div class="sec">// CONFIG B</div><div class="cfg-row"><div class="cfg-item"><label id="dlB">💎 DESIRED</label><input type="number" id="desB" value="5000" min="1" step="2" oninput="upCalc('B')"/><div class="hint" id="htB">1 click = 2 gems</div></div><div class="cfg-item"><label>⚡ WORKERS</label><input type="number" id="wrkB" value="20" min="1" max="200"/><div class="presets" id="preB"></div></div></div><div class="bg-notice">🌌 Background mein chalta hai — phone band karo!</div></div>
  ...
  <div class="card prog-card" id="progB"><div class="cg pk"></div><div class="sec">// JOB B PROGRESS</div>
    <div class="prog-top">
      <div class="ring-wrap">
        <svg viewBox="0 0 90 90">
  ...
  <div class="card"><div class="cg p"></div><div class="sec">// JOB B HISTORY</div><div class="sess-list" id="histB"><div class="empty">NO JOBS YET <span class="blink">_</span></div></div></div>
</div>
...
const COLS={gems:'var(--cgems)',tickets:'var(--ctickets)',elite:'var(--celite)',legendary:'var(--clegend)',champion:'var(--cchamp)'};
const ICONS={gems:'💎',tickets:'🎫',elite:'🃏',legendary:'⭐',champion:'👑'};
const MK=['gems','tickets','elite','legendary','champion'];
const MI=['💎','🎫','🃏','⭐','👑'];
const ML=['GEMS','TICKETS','ELITE','LEGEND','CHAMP'];
const MS=['+2/click','+30/click','+1/click','AUTO⚡','AUTO⚡'];
...
function setMode(s,m){
  ...
  document.getElementById('ht'+s).textContent=mc.hint;
  document.getElementById('des'+s).value=m==='tickets'?'300':m==='legendary'||m==='champion'?'10':'5000';
  document.getElementById('des'+s).step=m==='tickets'?'30':m==='legendary'||m==='champion'?'1':'2';
  const isB=s==='B';
  const btnCls=isB&&m==='gems'?'b':mc.cls||'';
...
function initVault(s){
  ...
  function draw(){
    ctx.fillStyle='rgba(5,0,10,0.08)';ctx.fillRect(0,0,cv.width,cv.height);
    drops.forEach((y,i)=>{
      const c=CH[Math.floor(Math.random()*CH.length)];
      ctx.fillStyle=Math.random()>.9?'#c264fe':'#33006688';
      ctx.font='9px Share Tech Mono';ctx.fillText(c,i*10,y*10);
      if(y*10>cv.height&&Math.random()>.97)drops[i]=0;drops[i]++;
    });
  }
...
if __name__ == "__main__":
    print("\033[35m")
    print("╔══════════════════════════════════════════╗")
    print("║  🌌 DC25 GALAXY FARM v6.1                ║")
    print("║  Purple Cosmic Theme — Full Galaxy!      ║")
