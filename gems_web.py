...
def run_job(sid, token, total, workers, mode_key):
    ...
    for b in range(batches):
        ...
        with ThreadPoolExecutor(max_workers=sz) as ex:
            fs = [ex.submit(one_req, hdr, sid, mode_key) for _ in range(sz)]
    ...
    with sessions_lock:
        if sid in sessions_data:
            ...
            sessions_data[sid]["history"].insert(0, {
                "reward":  sessions_data[sid]["success"] * m["amount"],
                "unit":    m["unit"],
                "label":   m["label"],
...
def start():
    ...
    threading.Thread(target=run_job, args=(sid,token,clicks,workers,mode_key), daemon=True).start()
    return jsonify({"ok":True,"clicks":clicks,"amount_per_click":m["amount"],"unit":m["unit"]})
...
.tl-ok{color:#00ff88;}.tl-w{color:#ffd600;}
...
.pt2{background:rgba(0,255,100,0.04);border:1px solid #0a2015;border-radius:2px;height:8px;overflow:hidden;}
...
.sl2{width:90px;text-align:right;}.st2{flex:1;height:3px;background:rgba(0,255,100,0.04);border-radius:1px;overflow:hidden;}
...
<div id="intro">
  ...
  <div id="termPanel">
    <div class="ptt">// BOOT LOG</div>
    <div id="tLines"></div>
  </div>
  <div id="dataPanel">
    <div class="dr"><span class="dk">NODE</span><span class="dv">RENDER-64</span></div>
    <div class="dr"><span class="dk">VERSION</span><span class="dv lv">v3.1.0</span></div>
    <div class="dr"><span class="dk">WORKERS</span><span class="dv lv">200</span></div>
    <div class="dr"><span class="dk">MODES</span><span class="dv lv">2</span></div>
    ...
    <div class="dr"><span class="dk">TICKETS</span><span class="dv lv" id="tk2">--</span></div>
  ...
  <div id="center">
    ...
    <div class="vtag">GEMS + TICKETS EDITION · v3.1</div>
    ...
    <div id="pw">
      ...
      <div class="sps">
        <div class="sr"><span class="sl2">CORE ENGINE</span><div class="st2"><div class="sf g" id="s1"></div></div><span class="sp2" id="s1p">0%</span></div>
        <div class="sr"><span class="sl2">NETWORK</span><div class="st2"><div class="sf c" id="s2"></div></div><span class="sp2" id="s2p">0%</span></div>
        <div class="sr"><span class="sl2">THREADS</span><div class="st2"><div class="sf y" id="s3"></div></div><span class="sp2" id="s3p">0%</span></div>
        <div class="sr"><span class="sl2">SECURITY</span><div class="st2"><div class="sf p" id="s4"></div></div><span class="sp2" id="s4p">0%</span></div>
      </div>
    </div>
    <div id="bs">▌</div>
  </div>
...
const cv=document.getElementById('matrix');const cx=cv.getContext('2d');
cv.width=window.innerWidth;cv.height=window.innerHeight;
const cols=Math.floor(cv.width/14);const drops=Array(cols).fill(1);
...
setInterval(()=>{document.getElementById('pv').textContent=(Math.floor(Math.random()*30)+10)+'ms';document.getElementById('gr2').textContent=Math.floor(Math.random()*12+18)+'/s';document.getElementById('tk2').textContent=Math.floor(Math.random()*3+1)+'/s';},900);
const tls=[{t:'BIOS v2.1.0 INITIALIZED',c:''},{t:'CPU: ARM64 x8 DETECTED',c:''},{t:'RAM: 512MB ALLOCATED',c:'ok'},{t:'LOADING KERNEL MODULES...',c:''},{t:'net0: INTERFACE UP',c:'ok'},{t:'FIREWALL: BYPASSED !!!',c:'warn'},{t:'SSL HANDSHAKE COMPLETE',c:'ok'},{t:'GEMS MODULE LOADED',c:'ok'},{t:'TICKETS MODULE LOADED',c:'ok'},{t:'200 THREADS SPAWNED',c:'ok'},{t:'ALL SYSTEMS READY',c:'ok'},];
const tEl=document.getElementById('tLines');let ti=0;
const tInt=setInterval(()=>{if(ti>=tls.length){clearInterval(tInt);return;}const l=tls[ti++];const el=document.createElement('div');const col=l.c==='ok'?'tl-ok':l.c==='warn'?'tl-w':'';el.innerHTML=`<span style="color:#0d3a1a">&gt;</span> <span class="${col}">${l.t}</span>`;el.style.cssText='opacity:0;animation:fUp .1s ease forwards;font-size:8px;line-height:1.9;';tEl.appendChild(el);tEl.scrollTop=tEl.scrollHeight;},240);
const dms=['DECRYPTING PAYLOAD...','LOADING GEMS ENGINE...','LOADING TICKETS ENGINE...','ESTABLISHING CHANNEL...','SYSTEM ARMED...'];
const hx='0123456789ABCDEF';
function decrypt(target,cb){const el=document.getElementById('dtxt');let it=0;const iv=setInterval(()=>{el.textContent=target.split('').map((c,i)=>i<it?c:c===' '?' ':hx[Math.floor(Math.random()*16)]).join('');it+=1.5;if(it>=target.length){el.textContent=target;clearInterval(iv);if(cb)setTimeout(cb,600);}},40);}
let di=0;function nextD(){if(di>=dms.length)return;decrypt(dms[di++],di<dms.length?nextD:null);}
setTimeout(nextD,2200);
const lbls=['LOADING CORE...','SPAWNING THREADS...','CONNECTING...','GEMS MODULE...','TICKETS MODULE...','CALIBRATING...','ARMED','READY'];
let pct=0,li=0;
...
function showAG(){const ag=document.getElementById('ag');ag.style.opacity='1';ag.style.pointerEvents='auto';let f=0;const iv=setInterval(()=>{f++;ag.style.background=f%2===0?'#000':'rgba(0,255,100,0.04)';if(f>=6){clearInterval(iv);setTimeout(showLogin,500);}},130);}
function showLogin(){const intro=document.getElementById('intro');const ag=document.getElementById('ag');const skip=document.getElementById('skipB');const lw=document.getElementById('loginWrap');[intro,ag].forEach(el=>{el.style.transition='opacity 0.7s';el.style.opacity='0';});skip.style.display='none';clearInterval(mInt);setTimeout(()=>{intro.style.display='none';ag.style.display='none';lw.classList.add('show');document.getElementById('em').focus();},750);}
...
async function doLogin(){
  ...
  try{const r=await fetch('/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password:pass})});const d=await r.json();if(d.ok){window.location.reload();}else{showErr(d.error||'Access denied!');}}catch(e){showErr('Connection error!');}
...
/* MODE SELECTOR */
.mode-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:4px;}
.mode-btn{padding:14px 10px;background:rgba(0,255,100,.03);border:1px solid var(--border);border-radius:5px;cursor:pointer;transition:all .2s;text-align:center;position:relative;overflow:hidden;}
.mode-btn:hover{border-color:var(--g2);}
...
.mode-label{font-family:'Orbitron',monospace;font-size:9px;letter-spacing:3px;color:var(--dim);}
.mode-btn.active.gems .mode-label{color:var(--g1);}
.mode-btn.active.tickets .mode-label{color:var(--orange);}
.mode-sub{font-size:9px;color:var(--dim);margin-top:3px;}
.mode-btn.active.gems .mode-sub{color:var(--g2);}
.mode-btn.active.tickets .mode-sub{color:var(--orange);}
...
.btn-start.tickets-mode{background:linear-gradient(135deg,#2a1400,#503000,#3a1c00);border-color:var(--orange);color:var(--orange);text-shadow:0 0 15px rgba(255,140,0,.6);}
...
@keyframes shine{0%{left:-60%}100%{left:160%}}
.btn-start:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 0 50px rgba(0,255,100,.2);}
...
.bar-fill.tickets{background:linear-gradient(90deg,#3a1c00,#995500,#ff8c00);box-shadow:0 0 10px rgba(255,140,0,.4);}
...
.sv.c{color:var(--cyan);}.sv.y{color:var(--yellow);}.sv.r{color:var(--red);}.sv.o{color:var(--orange);}
...
.done-card.tickets{background:rgba(255,140,0,.04);border-color:var(--orange);}
...
.done-title.tickets{color:var(--orange);}
...
.si.tickets{background:rgba(255,140,0,.02);border-color:rgba(255,140,0,.15);}
.si-gems{font-family:'Orbitron',monospace;font-size:16px;font-weight:700;color:var(--g1);}
.si-gems.tickets{color:var(--orange);}
...
<div class="hdr">
  ...
  <h1>ULTRA FARMER</h1>
  <p>💎 GEMS + 🎫 WORLD CUP TICKETS · 200 WORKERS</p>
...
<div class="page">
  ...
  <div class="card">
    <div class="cg o"></div>
    <div class="sec">// SELECT MODE</div>
    <div class="mode-row">
      <div class="mode-btn active gems" id="mGems" onclick="setMode('gems')">
        <div class="mode-icon">💎</div>
        <div class="mode-label">GEMS</div>
        <div class="mode-sub">+2 per click</div>
      </div>
      <div class="mode-btn tickets" id="mTickets" onclick="setMode('tickets')">
        <div class="mode-icon">🎫</div>
        <div class="mode-label">WORLD CUP</div>
        <div class="mode-sub">+30 tickets per click</div>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="cfg-row">
      <div class="cfg-item">
        <label id="desiredLabel">💎 DESIRED GEMS</label>
        <input type="number" id="desired" value="5000" min="2" step="2"/>
        <div class="hint" id="hintTxt">1 click = 2 gems</div>
      </div>
      <div class="cfg-item">
  ...
  <div class="card prog-card" id="progCard">
    ...
    <div class="prog-top">
      ...
      <div class="prog-info">
        <div class="plbl" id="rewardLabel">GEMS ADDED</div>
        <div class="pval" id="pReward">0</div>
  ...
  <div class="done-card" id="doneCard">
    ...
    <div class="done-title" id="doneTitle">FARMING COMPLETE</div>
...
let poll=null,currentMode='gems';

function setMode(m){
  currentMode=m;
  document.getElementById('mGems').className='mode-btn'+(m==='gems'?' active gems':' gems');
  document.getElementById('mTickets').className='mode-btn'+(m==='tickets'?' active tickets':' tickets');
  const isT=m==='tickets';
  document.getElementById('desiredLabel').textContent=isT?'🎫 DESIRED TICKETS':'💎 DESIRED GEMS';
  document.getElementById('hintTxt').textContent=isT?'1 click = 30 tickets':'1 click = 2 gems';
  document.getElementById('desired').value=isT?'300':'5000';
  document.getElementById('desired').step=isT?'30':'2';
  const btn=document.getElementById('btnS');
  btn.textContent=isT?'▶ LAUNCH TICKET FARMING':'▶ LAUNCH FARMING';
  btn.className='btn-start'+(isT?' tickets-mode':'');
}
...
function ring(p,isT){
  ...
  document.getElementById('ring').className='ring-fg'+(isT?' tickets':'');
  document.getElementById('rpct').textContent=p.toFixed(1)+'%';
  document.getElementById('rpct').className='ring-pct'+(isT?' tickets':'');
}
function graph(h,isT){const w=document.getElementById('gBars');if(!h.length)return;const mx=Math.max(...h,1);w.innerHTML=h.map(v=>`<div class="gb${isT?' tickets':''}" style="height:${Math.max(4,(v/mx)*44)}px"></div>`).join('');}
function renderSess(list){
  const el=document.getElementById('sessList');
  if(!list.length){el.innerHTML='<div class="empty">NO SESSIONS YET <span class="cur2">_</span></div>';return;}
  el.innerHTML=list.map(s=>{
    const isT=s.unit==='Tickets';
    return `<div class="si${isT?' tickets':''}">
      <div>
        <div class="si-gems${isT?' tickets':''}">+${s.reward} ${s.label}</div>
        <div class="si-meta">${s.success}/${s.total} clicks · ${s.workers}w</div>
      </div>
...
async function go(){
  ...
  document.getElementById('bar').className='bar-fill'+(isT?' tickets':'');
  document.getElementById('pReward').textContent='0';
  document.getElementById('pReward').className='pval'+(isT?' tickets':'');
  document.getElementById('rewardLabel').textContent=isT?'TICKETS ADDED':'GEMS ADDED';
  ...
  const res=await fetch('/start',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({token:tok,desired,workers:wrk,sid:SID,mode:currentMode})});
  const data=await res.json();
  if(data.error){alert(data.error);resetUI();return;}
  setSt(`Running ${data.clicks} clicks · ${wrk} workers · ${data.unit}`,'g');
...
async function tick(){
  try{
    ...
    document.getElementById('bar').className='bar-fill'+(isT?' tickets':'');
    document.getElementById('pReward').textContent=d.reward;
    ...
    renderSess(d.history);
    if(d.running)setSt(`${d.completed}/${d.total} · ${fmt(d.elapsed)} elapsed`,'g');
    if(d.done){
      clearInterval(poll);resetUI();ring(100,isT);
      document.getElementById('bar').style.width='100%';
      const dc=document.getElementById('doneCard');
      dc.style.display='block';
      dc.className='done-card'+(isT?' tickets':'');
      document.getElementById('doneIcon').textContent=isT?'🎫':'💎';
      document.getElementById('doneTitle').className='done-title'+(isT?' tickets':'');
      const col=isT?'var(--orange)':'var(--g1)';
      document.getElementById('doneSub').innerHTML=`<strong style="color:${col}">+${d.reward} ${d.unit}</strong> added!<br/>${d.success}/${d.total} success · ${fmt(d.elapsed)} total`;
      setSt('Complete!','g');
    }
...
if __name__ == "__main__":
    print(f"\033[92m")
    print("╔══════════════════════════════════════════╗")
    print("║  DC25 ULTRA FARMER - GEMS + TICKETS      ║")
