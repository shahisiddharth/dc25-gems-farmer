"""
Dream Cricket 25 - ULTRA Farmer v4.0
- Persistent Background Jobs (browser band ho to bhi chalta hai)
- Auto-ping (Render free tier never sleeps)
- Emergency Stop anytime
- Gems + World Cup Tickets
- Hacker Intro + Login
"""
...
# ── Persistent job store — survives browser close ──
...
def auto_ping():
    ...
    while True:
        ...
        try:
            req.get(f"{SELF_URL}/ping", timeout=10)
        except:
            pass
...
def run_job(job_id):
    """Runs in background thread — independent of browser"""
    with jobs_lock:
    ...
    with jobs_lock:
        if job_id in jobs:
            ...
            with hist_lock:
                completed_history.insert(0, entry)
...
def start():
    ...
    return jsonify({"ok":True, "job_id":job_id, "clicks":clicks, "unit":m["unit"], "amount":m["amount"]})
...
def status():
    ...
    if not j:
        return jsonify({
            ...
            "speed_history":[],"history":hist,"job_id":"",
            "has_active":False,
        })
...
@app.route("/active")
def active():
    """Check if any job is running — used on page load"""
    if not is_logged_in(): return jsonify({"error":"Unauthorized"}), 401
    jid, j = active_job()
    if jid and j:
        m = MODES.get(j["mode_key"], MODES["gems"])
...
#tp{position:absolute;left:14px;top:50%;transform:translateY(-50%);width:min(200px,24vw);background:rgba(0,255,100,.03);border:1px solid rgba(0,255,100,.08);border-radius:4px;padding:10px;font-size:8px;color:#1a5a2a;line-height:1.9;opacity:0;animation:fu .5s ease 1.2s forwards;max-height:65vh;overflow:hidden;}
.ptt{color:#00aa55;border-bottom:1px solid #0d2a1a;padding-bottom:5px;margin-bottom:7px;letter-spacing:3px;font-size:7px;}
.ok{color:#00ff88;}.wn{color:#ffd600;}
#dp{position:absolute;right:14px;top:50%;transform:translateY(-50%);font-size:8px;color:#1a4a2a;line-height:2.1;opacity:0;animation:fu .5s ease 1.4s forwards;text-align:right;}
.dr{display:flex;justify-content:flex-end;gap:8px;}.dk{color:#1a3a2a;}.dv{color:#00cc66;font-size:9px;}
.lv{color:#00ffaa!important;animation:vp 2s infinite;}
...
.mt{font-family:'Orbitron',monospace;font-weight:900;font-size:clamp(24px,7vw,52px);letter-spacing:8px;color:#00ffaa;text-shadow:0 0 30px rgba(0,255,136,.6),0 0 60px rgba(0,255,136,.25);position:relative;}
...
.dt{font-size:10px;color:#00aa44;letter-spacing:3px;margin-top:8px;min-height:16px;}
...
.pf{height:100%;width:0%;background:linear-gradient(90deg,#002a14,#00aa55,#00ffaa);box-shadow:0 0 12px rgba(0,255,100,.4);}
...
.agt{font-family:'Orbitron',monospace;font-weight:900;font-size:clamp(16px,5vw,32px);letter-spacing:8px;color:#00ffaa;text-shadow:0 0 30px rgba(0,255,136,1);}
.ags{font-size:10px;color:#00aa55;letter-spacing:4px;margin-top:6px;}
#sk{position:fixed;bottom:18px;right:18px;z-index:100;background:rgba(0,255,100,.04);border:1px solid rgba(0,255,100,.12);border-radius:3px;color:rgba(0,255,100,.25);font-family:'Share Tech Mono',monospace;font-size:9px;letter-spacing:2px;padding:5px 12px;cursor:pointer;transition:all .2s;}
#sk:hover{color:rgba(0,255,100,.6);}
...
.lf input:focus{border-color:#00cc77;box-shadow:0 0 12px rgba(0,255,100,.1);}
.lb{width:100%;padding:15px;margin-top:6px;background:linear-gradient(135deg,#002a14,#005028,#003a1c);border:1px solid #00cc77;border-radius:6px;color:#00ffaa;font-family:'Orbitron',monospace;font-weight:900;font-size:13px;letter-spacing:5px;cursor:pointer;transition:all .2s;text-shadow:0 0 12px rgba(0,255,100,.5);position:relative;overflow:hidden;}
...
<div id="intro">
  ...
  <div id="tb">
    ...
    <span style="color:#0d3a1a">|</span><div class="dot y"></div>ENCRYPTION:AES-256
    <span style="color:#0d3a1a">|</span><div class="dot r"></div>FIREWALL:BYPASSED
  </div>
  <div id="tp"><div class="ptt">// BOOT LOG</div><div id="tl"></div></div>
  <div id="dp">
    <div class="dr"><span class="dk">NODE</span><span class="dv">RENDER-64</span></div>
    <div class="dr"><span class="dk">VERSION</span><span class="dv lv">v4.0.0</span></div>
    <div class="dr"><span class="dk">WORKERS</span><span class="dv lv">200</span></div>
    <div class="dr"><span class="dk">AUTO-PING</span><span class="dv lv">ON ✓</span></div>
    <div class="dr"><span class="dk">BG JOBS</span><span class="dv lv">ENABLED</span></div>
  ...
  <div id="ctr">
    ...
    <div class="vt">BG JOBS + AUTO-PING · v4.0</div>
    ...
    <div id="pw">
      ...
      <div class="sps">
        ...
        <div class="sr"><span class="sl">BG JOBS</span><div class="sts"><div class="sf y" id="s3"></div></div><span class="sp" id="s3p">0%</span></div>
...
setInterval(()=>{document.getElementById('pv').textContent=(Math.floor(Math.random()*25)+8)+'ms';},800);
const tls=[{t:'BIOS v2.1.0 INITIALIZED',c:''},{t:'CPU: ARM64 x8',c:''},{t:'RAM: 512MB ALLOCATED',c:'ok'},{t:'AUTO-PING: ENABLED ✓',c:'ok'},{t:'BG JOBS ENGINE: READY',c:'ok'},{t:'FIREWALL: BYPASSED',c:'wn'},{t:'SSL: CONNECTED',c:'ok'},{t:'GEMS MODULE LOADED',c:'ok'},{t:'TICKETS MODULE LOADED',c:'ok'},{t:'200 THREADS SPAWNED',c:'ok'},{t:'ALL SYSTEMS GO',c:'ok'},];
const tEl=document.getElementById('tl');let ti=0;
const tInt=setInterval(()=>{if(ti>=tls.length){clearInterval(tInt);return;}const l=tls[ti++];const el=document.createElement('div');el.innerHTML=`<span style="color:#0d3a1a">&gt;</span> <span class="${l.c==='ok'?'ok':l.c==='wn'?'wn':''}">${l.t}</span>`;el.style.cssText='opacity:0;animation:fu .1s ease forwards;font-size:8px;line-height:1.9;';tEl.appendChild(el);tEl.scrollTop=tEl.scrollHeight;},230);
const dms=['DECRYPTING PAYLOAD...','BG JOBS ENGINE INIT...','AUTO-PING ARMED...','GEMS MODULE READY...','TICKETS MODULE READY...'];
const hx='0123456789ABCDEF';
function decrypt(t,cb){const el=document.getElementById('dt');let it=0;const iv=setInterval(()=>{el.textContent=t.split('').map((c,i)=>i<it?c:c===' '?' ':hx[Math.floor(Math.random()*16)]).join('');it+=1.5;if(it>=t.length){el.textContent=t;clearInterval(iv);if(cb)setTimeout(cb,600);}},40);}
let di=0;function nD(){if(di>=dms.length)return;decrypt(dms[di++],di<dms.length?nD:null);}
setTimeout(nD,2200);
let pct=0,li=0;
const lbls=['LOADING CORE...','AUTO-PING INIT...','BG JOBS ENGINE...','GEMS+TICKETS...','CALIBRATING...','ARMED','READY'];
...
setTimeout(()=>{const iv=setInterval(()=>{pct+=Math.random()*3.5+0.5;if(pct>=100){pct=100;clearInterval(iv);setTimeout(showAG,300);}pfl.style.width=pct+'%';pn.textContent=Math.floor(pct)+'%';const nl=Math.floor((pct/100)*lbls.length);if(nl!==li&&nl<lbls.length){li=nl;plbl.textContent=lbls[li];bsEl.textContent='> '+lbls[li];}[['s1','s1p'],['s2','s2p'],['s3','s3p'],['s4','s4p']].forEach(([id,pid],i)=>{const v=Math.min(100,Math.max(0,(pct-i*25)*4));document.getElementById(id).style.width=v+'%';document.getElementById(pid).textContent=Math.floor(v)+'%';});},55);},2600);
...
<head>
<meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>DC25 ULTRA FARMER v4</title>
...
.logout-btn{position:absolute;top:16px;right:14px;background:transparent;border:1px solid rgba(255,34,68,.3);border-radius:3px;color:var(--red);font-family:'Share Tech Mono',monospace;font-size:10px;padding:4px 9px;cursor:pointer;transition:all .2s;}
.logout-btn:hover{background:rgba(255,34,68,.1);}

/* PING indicator */
.ping-badge{position:absolute;top:16px;left:14px;display:flex;align-items:center;gap:5px;font-size:9px;color:var(--g2);letter-spacing:1px;}
...
/* Active job banner */
#activeBanner{display:none;background:rgba(0,255,100,.06);border:1px solid var(--g2);border-radius:6px;padding:14px 16px;margin:14px 14px 0;}
...
.ab-info{font-size:12px;color:var(--text);}
.ab-stop{background:rgba(255,34,68,.1);border:1px solid var(--red);border-radius:4px;color:var(--red);font-family:'Share Tech Mono',monospace;font-size:11px;padding:6px 14px;cursor:pointer;letter-spacing:1px;transition:all .2s;white-space:nowrap;}
.ab-stop:hover{background:rgba(255,34,68,.2);}
.ab-bar{margin-top:8px;background:rgba(0,255,100,.05);border:1px solid var(--border);border-radius:2px;height:6px;overflow:hidden;}
.ab-fill{height:100%;background:linear-gradient(90deg,var(--g3),var(--g2),var(--g1));transition:width .5s;}

.page{max-width:680px;margin:0 auto;padding:14px 14px 40px;display:flex;flex-direction:column;gap:12px;}
...
textarea{width:100%;background:rgba(0,255,100,.03);border:1px solid var(--border);border-radius:4px;color:var(--g1);font-family:'Share Tech Mono',monospace;font-size:12px;padding:10px;outline:none;resize:none;height:80px;transition:border-color .2s;}
textarea:focus{border-color:var(--g2);box-shadow:0 0 10px rgba(0,255,100,.1);}
...
.mode-btn.active.gems{border-color:var(--g1);background:rgba(0,255,100,.08);box-shadow:0 0 20px rgba(0,255,100,.1);}
.mode-btn.active.tickets{border-color:var(--orange);background:rgba(255,140,0,.08);box-shadow:0 0 20px rgba(255,140,0,.1);}
.mi{font-size:26px;margin-bottom:5px;}
...
.mode-btn.active.gems .ms{color:var(--g2);}
.mode-btn.active.tickets .ms{color:var(--orange);}
...
.btn-start{width:100%;padding:16px;background:linear-gradient(135deg,#002a14,#005028,#003a1c);border:1px solid var(--g2);border-radius:6px;color:var(--g1);font-family:'Orbitron',monospace;font-weight:900;font-size:13px;letter-spacing:5px;cursor:pointer;transition:all .2s;position:relative;overflow:hidden;text-shadow:0 0 15px rgba(0,255,100,.6);box-shadow:0 0 30px rgba(0,255,100,.08);}
.btn-start.tm{background:linear-gradient(135deg,#2a1400,#503000,#3a1c00);border-color:var(--orange);color:var(--orange);text-shadow:0 0 15px rgba(255,140,0,.6);}
...
.ring-wrap{position:relative;width:85px;height:85px;flex-shrink:0;}
.ring-wrap svg{width:85px;height:85px;transform:rotate(-90deg);}
...
.graph{background:rgba(0,255,100,.03);border:1px solid var(--border);border-radius:3px;height:48px;overflow:hidden;}
...
/* BG Job notice */
.bg-notice{background:rgba(0,229,255,.05);border:1px solid rgba(0,229,255,.2);border-radius:4px;padding:10px 14px;font-size:11px;color:#00e5ff;letter-spacing:1px;text-align:center;margin-top:4px;}
</style>
</head>
<body>
<div class="wrap">
<div class="hdr">
  <div class="ping-badge"><div class="ping-dot"></div>AUTO-PING ON</div>
  <div class="hdr-badge">DREAM CRICKET 25</div>
  <h1>ULTRA FARMER v4</h1>
  <p>💎 GEMS · 🎫 TICKETS · BG JOBS · AUTO-PING</p>
  <button class="logout-btn" onclick="doLogout()">⏻ LOGOUT</button>
</div>

<!-- Active job banner (shown if job running) -->
<div id="activeBanner">
...
<div class="page">
  ...
  <div class="card">
    ...
    <div class="mode-row">
      <div class="mode-btn active gems" id="mG" onclick="setMode('gems')">
        <div class="mi">💎</div><div class="ml">GEMS</div><div class="ms">+2 per click</div>
      </div>
      <div class="mode-btn tickets" id="mT" onclick="setMode('tickets')">
        <div class="mi">🎫</div><div class="ml">WORLD CUP</div><div class="ms">+30 per click</div>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="cfg-row">
      <div class="cfg-item">
        ...
        <input type="number" id="desired" value="5000" min="2" step="2"/>
    ...
    <div class="bg-notice">🔁 Job server pe chalta hai — phone/browser band karo, chalta rahega!</div>
  </div>
  ...
  <div class="card prog-card" id="progCard">
    <div class="cg" id="pCg"></div><div class="sec">// LIVE PROGRESS</div>
    <div class="prog-top">
      <div class="ring-wrap">
  ...
  <!-- History -->
  <div class="card">
    <div class="cg p"></div><div class="sec">// JOB HISTORY (last 10)</div>
...
<script>
const CIRC=245;
let poll=null,currentMode='gems',currentJobId='';

// On page load — check if any job running
...
function showActiveBanner(d){
  ...
  document.getElementById('abInfo').textContent=
    `${isT?'🎫':'💎'} ${d.reward} ${d.unit} added · ${d.completed}/${d.total} clicks · ${fmt(d.elapsed)} elapsed`;
...
function graph(h,isT){const w=document.getElementById('gBars');if(!h.length)return;const mx=Math.max(...h,1);w.innerHTML=h.map(v=>`<div class="gb${isT?' t':''}" style="height:${Math.max(3,(v/mx)*42)}px"></div>`).join('');}
function renderHist(list){
  ...
  el.innerHTML=list.map(s=>{
    ...
    return `<div class="si${isT?' t':''}">
      ...
      <div class="si-m">${s.success}/${s.total} clicks · ${s.workers}w</div></div>
...
async function go(){
  ...
  setSt(`Background job started · ${data.clicks} clicks · ${wrk} workers`,'g');
...
async function tick(){
  try{
    ...
    if(d.has_active){
      showActiveBanner(d);
      setSt(`${d.completed}/${d.total} · ${fmt(d.elapsed)} elapsed · browser band karo, chalta rahega! ✓`,'g');
    }
    if(d.done){
      clearInterval(poll);resetUI();hideActiveBanner();ring(100,isT);
      document.getElementById('bar').style.width='100%';
      const dc=document.getElementById('doneCard');
      dc.style.display='block';dc.className='done-card'+(isT?' t':'');
...
async function stopJob(){
  ...
  clearInterval(poll);setSt('Job stopped.','r');resetUI();hideActiveBanner();
}

async function emergencyStop(){
  if(!confirm('Job stop karna hai?')) return;
  await fetch('/stop',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({job_id:currentJobId})});
  clearInterval(poll);setSt('⛔ Emergency stop!','r');resetUI();hideActiveBanner();
}
...
if __name__ == "__main__":
    print(f"\033[92m")
    print("╔══════════════════════════════════════════╗")
    print("║  DC25 ULTRA FARMER v4.0                  ║")
    print("║  BG Jobs + Auto-Ping + Gems + Tickets    ║")
