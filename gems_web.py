...
NEW: Gems = 10 per click (was 2) — 5x FASTER!
"""
...
MODES = {
    # ✅ NEW METHOD: 10 gems per click (was 2)
    "gems": {
    ...
    "tickets": {
        "label":"🎫 Tickets", "unit":"Tickets", "type":"reward",
        "templateId":124339,"currencyTypeId":23, "amount":30,
    },
    "elite": {
        "label":"🃏 Elite",  "unit":"Elite",   "type":"reward",
        "templateId":122012,"currencyTypeId":14, "amount":1,
    },
    "legendary": {
...
# ── NEW GEMS MUTATION (10 per click) ──
...
def run_job(slot):
    ...
    if m["type"] == "chain":
        ...
        with lk:
            j = slots[slot]["job"]
            if j:
                j["phase"] = 1; j["phase1_total"] = elite_needed
                j["phase2_total"] = total; j["phase_done"] = 0
        p1ok = run_phase(slot, hdr, elite_needed, workers,
...
def get_slot_status(slot):
    ...
    if not j:
        return {"running":False,"done":False,"slot":slot,"pct":0,"reward":0,"unit":"",
            ...
            "phase_done":0,"phase1_success":0,"phase2_success":0,"is_chain":False,
            "mode_key":"gems","total":0}
...
.lf input:focus{border-color:#c264fe;}
.lb{width:100%;padding:14px;margin-top:5px;background:linear-gradient(135deg,#1a0028,#350050,#28003a);border:1px solid #9933ff;border-radius:6px;color:#c264fe;font-family:'Orbitron',monospace;font-weight:900;font-size:12px;letter-spacing:5px;cursor:pointer;position:relative;overflow:hidden;}
...
<canvas id="cv"></canvas>
<div class="nebula" style="width:400px;height:400px;background:#6600aa;top:10%;left:20%;"></div>
...
<div id="intro">
  ...
  <div id="ctr">
    ...
    <div class="vt">COSMIC MODE · DC25 · v6.2</div>
    <div class="new-badge">⚡ NEW: 10 GEMS/CLICK · 5X FASTER</div>
    <div id="pw">
      <div class="ph"><span id="pl">INITIALIZING...</span><span id="pn">0%</span></div>
      <div class="pt2"><div class="pf" id="pfl"></div></div>
      <div class="sps">
        <div class="sr"><span class="sl2">NEW GEM METHOD</span><div class="sts"><div class="sf" id="s1" style="background:#00ff88"></div></div><span class="sp" id="s1p">0%</span></div>
        <div class="sr"><span class="sl2">DUAL JOBS</span><div class="sts"><div class="sf" id="s2"></div></div><span class="sp" id="s2p">0%</span></div>
...
let pct=0,li=0;
const lbls=['NEW METHOD 10x...','DUAL JOBS...','AUTO-CHAIN...','COSMIC PING...','READY!'];
...
<div class="hdr">
  ...
  <div class="new-method">⚡ NEW: GEMS 10/CLICK · 5X FASTER · v6.2</div>
...
<div class="job-panel show" id="panelA">
  ...
  <div class="card"><div class="cg cy"></div><div class="sec">// CONFIG A</div>
    <div class="cfg-row">
      <div class="cfg-item"><label id="dlA">💎 DESIRED GEMS</label><input type="number" id="desA" value="5000" min="1" step="10" oninput="upCalc('A')"/><div class="hint new" id="htA">⚡ 1 click = 10 gems (NEW!)</div></div>
...
<div class="job-panel" id="panelB">
  ...
  <div class="card"><div class="cg cy"></div><div class="sec">// CONFIG B</div>
    <div class="cfg-row">
      <div class="cfg-item"><label id="dlB">💎 DESIRED GEMS</label><input type="number" id="desB" value="5000" min="1" step="10" oninput="upCalc('B')"/><div class="hint new" id="htB">⚡ 1 click = 10 gems (NEW!)</div></div>
...
const CIRC=245;
const MCFG={
  gems:     {lbl:'💎 DESIRED GEMS',   hint:'⚡ 1 click = 10 gems (NEW!)', hintNew:true, btn:'LAUNCH', cls:'',        chain:false,icon:'💎',epc:0},
  tickets:  {lbl:'🎫 DESIRED TICKETS',hint:'1 click = 30 tickets',        hintNew:false,btn:'LAUNCH', cls:'tickets', chain:false,icon:'🎫',epc:0},
  elite:    {lbl:'🃏 DESIRED ELITE',   hint:'1 click = 1 card',            hintNew:false,btn:'LAUNCH', cls:'elite',   chain:false,icon:'🃏',epc:0},
...
const MS=['10/click⚡','+30/click','+1/click','AUTO⚡','AUTO⚡'];
...
async function go(s){
  ...
  setSt(s,data.is_chain?`🌌 AUTO-CHAIN: ${data.elite_needed} elite → ${desired} cards`:`🌌 Job ${s} started — 10 gems/click ⚡`,'g');
...
if __name__ == "__main__":
    print("\033[35m")
    print("╔══════════════════════════════════════════╗")
    print("║  🌌 DC25 GALAXY FARM v6.2                ║")
    print("║  ⚡ NEW: 10 GEMS/CLICK (was 2) — 5X!    ║")
